...
const APICARTOONS_URL = `https://bazon.cc/api/json?token=${API_TOKEN}&type=film&page=1&cat=мультфильм&year=${new Date().getFullYear()}`;
...
module.exports = new Promise(function(resolve, reject){
  ...
  // из полученных данных создаю массив с html блоками
    const item = data.results.map((elem, index) => {
       return `
        <div id="cartoon${index}" class="cartoonsItem item nav-item" data-nav_ud="0,none,0,none">
        ...
        <script type="text/javascript">
            ...
            } = document.getElementById("cartoon${index}")
            _elem${
              elem.kinopoisk_id + index
